...
func newDialer(ctx context.Context, source snapshotSource, options Options, build buildFunc) (*Dialer, error) {
	...
	return &Dialer{lifetime: life, cancel: cancel, pool: newPool(), descriptors: newDescriptorCache(), intros: newIntroductionRetainer(options.MaxCircuits), source: source, build: build, options: options, slots: make(chan struct{}, options.MaxCircuits), wait: waitBuildRetry}, nil
}
